...
#
.factory("auth", ($http, $cookieStore, $timeout, usernameCookie, currentUser, authRoles, localStorageService, doubtfireLoginTimeCookie, rememberDoubtfireCredentialsCookie, dfApiUrl, $rootScope) ->
  ...
  updateAuth = (authenticationUrl) ->
    ...
    $http.put(authenticationUrl,
      ...
      $timeout (( ) -> updateAuth dfApiUrl + "/auth/" + currentUser.authenticationToken + ".json"), 1000*60*60
    )
  ...
  if checkAuth()
    ...
    $timeout (( ) -> updateAuth dfApiUrl + "/auth/" + currentUser.authenticationToken + ".json"), delayTime
